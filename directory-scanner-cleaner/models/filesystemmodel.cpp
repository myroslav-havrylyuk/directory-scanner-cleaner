...
FileSystemModel::FileSystemModel()
    : m_FileTreeRoot(nullptr), m_FilesystemRootElement(nullptr),
    ...
    QObject::connect(&m_DeleteFilesFutureWatcher,
                     &QFutureWatcher<QList<QString>>::finished, this,
                     &FileSystemModel::handleDeleteSelectedFilesFinished);
}
...
QModelIndex FileSystemModel::parent(const QModelIndex &child) const {
    ...
    FileTreeElement *childTreeElement =
            static_cast<FileTreeElement *>(child.internalPointer());
...
int FileSystemModel::rowCount(const QModelIndex &parent) const {
    ...
    return (fileTreeElement == nullptr) ? 0 : fileTreeElement->getChildsCount();
}

int FileSystemModel::columnCount(const QModelIndex &parent) const {
    FileTreeElement *fileTreeElement = indexToFileTreeElement(parent);

    return (fileTreeElement == nullptr) ? 0 : fileTreeElement->getRolesCount();
}
...
bool FileSystemModel::removeRow(int row, const QModelIndex &parent) {
    ...
    while ((parentItem = parentItem->getParent()) != nullptr)
        emit dataChanged(createIndex(0, 0, parentItem),
...
bool FileSystemModel::hasChildren(const QModelIndex &parent) const {
    ...
    return (fileTreeElement == nullptr)
            ? false
            : (!fileTreeElement->getChildElements().empty());
}

bool FileSystemModel::hasIndex(
        int row, int column, const QModelIndex &parent = QModelIndex()) const {
    FileTreeElement *fileTreeElement = indexToFileTreeElement(parent);
    if (fileTreeElement == nullptr)
        return false;

    return (row >= 0 && row < fileTreeElement->getChildsCount() && column >= 0 &&
            column < fileTreeElement->getRolesCount());
}

QString FileSystemModel::getRootPath() {
    return m_FilesystemRootElement ? m_FilesystemRootElement->fileName()
                                   : QString();
}

void FileSystemModel::selectFileManually(QModelIndex index) {
    m_DeletionReasonsStringModel.setInitialDeletionReason(
                m_DeletionReasons[DeletionReason::OTHER]);
...
void FileSystemModel::connectToFileSystemManager() {
    connect(m_FileSystemManager, &FileSystemManager::fileTreeGenerated, this,
            &FileSystemModel::fileTreeGeneratedHandler);
    connect(m_FileSystemManager, &FileSystemManager::setupModelCanceled, this,
            &FileSystemModel::setupModelCanceledHandler);
    connect(this, &FileSystemModel::cancelSetupModel, m_FileSystemManager,
            &FileSystemManager::cancelSetupModelHandler);
}
...
void FileSystemModel::fileTreeGeneratedHandler(FileTreeElement *fileTreeRoot) {
    qDebug() << QTime::currentTime()
             << "handled fileTreeGenerated signal in FileSystemModel";
...
void FileSystemModel::cancelSetupModelHandler() {
    qDebug() << QTime::currentTime()
             << "FileSystemModel: cancel setup model handler";
...
void FileSystemModel::setupModelCanceledHandler() {
    ...
    qDebug() << QTime::currentTime()
             << "setup model canceled handler in FileSystemModel";
}

FileTreeElement *
FileSystemModel::indexToFileTreeElement(const QModelIndex &index) const {
    if (!index.isValid())
...
void FileSystemModel::deleteSelectedFiles() {
    ...
    // Fixes unpecified signal catching for QAbstractItemModel which is not
    // thread-safe by default. If bad timing occurs(which happens pretty often) we
    // will end up with invalid tree view displayed. Sole purpose of that connect
    // is to ensure handling of signal emmited by beginRemoveRows() in UI thread
    // before the next line executes which deletes data from model.
    connect(this, &QAbstractItemModel::rowsAboutToBeRemoved, this,
            &FileSystemModel::mockHandler,
            static_cast<Qt::ConnectionType>(Qt::BlockingQueuedConnection |
                                            Qt::UniqueConnection));

    m_DeleteFilesFuture =
            QtConcurrent::run(QOverload<QPromise<QList<QString>> &>::of(
    ...
    while (!m_ItemSelectionModel.selectedIndexes().empty()) {
        ...
        while (it.hasNext()) {
            ...
            if (promise.isCanceled()) {
                ...
                emit fileDeletionFinished(
                            deletedFilenames,
                            m_DeletionReasonsStringModel.getActiveDeletionReason());
                emit fileDeletionCancelingOperationFinished();
                return;
            }

            m_ItemSelectionModel.select(fileElementToDeleteModelIndex,
                                        QItemSelectionModel::Deselect);

            // Reverse iterator first try to delete all of the inner files in the
            // directory. When we iterate right to the holding folder and find it
            // empty we can just simply delete it. In case it still contains some
            // files that means some its inner files` deletion operation failed.
            ...
            if (m_FileSystemManager->deleteFile(fileElementAbsolutePath)) {
                ...
                removeRow(fileElementToDeleteModelIndex.row(),
                          fileElementToDeleteModelIndex.parent());
            }
        }
    }

    emit fileDeletionFinished(
                deletedFilenames, m_DeletionReasonsStringModel.getActiveDeletionReason());
}
...
void FileSystemModel::deselectFilesAsync() {
    ...
    *m_DeselectionFuture =
            QtConcurrent::run(&FileSystemModel::deselectFiles, this);
    QObject::connect(m_DeselectionWatcher, &QFutureWatcher<void>::finished, this,
                     &FileSystemModel::deselectionFinished);
