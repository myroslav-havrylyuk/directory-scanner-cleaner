...
MainWindowController::MainWindowController(QObject *parent) : QObject{parent} {
    ...
    m_MainQmlContext->setContextProperty(
                "DeletionReasonsStringModel",
                m_FileSystemModel->getDeletionReasonsStringModel());
    m_MainQmlContext->setContextProperty("FileSystemController",
                                         m_FileSystemController);
    ...
    QObject *deselectButton =
            m_MainWindow->findChild<QObject *>("deselect_button");
    ...
    QObject::connect(reloadButton, SIGNAL(reloadFileSystem(QString)), this,
                     SLOT(setActivePath(QString)));
}
...
void MainWindowController::updateDeletionHistory(
        const QList<QString> &deletedFilesList, const QString &fileDeletionReason) {
    m_FileDeletionHistoryManager.updateHistory(
                deletedFilesList, fileDeletionReason,
                gSettingsController->getHistoryPath() + "/filesDeletionHistory" + " - " +
                QDate::currentDate().toString("dd.MM.yyyy") + ".json");
}
